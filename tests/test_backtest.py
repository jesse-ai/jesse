...
def test_backtesting_one_route():
    ...
    one_min = store.candles.get_candles(exchanges.SANDBOX, 'BTCUSD', '1m')
    five_min = store.candles.get_candles(exchanges.SANDBOX, 'BTCUSD', '5m')
    ...
    p = selectors.get_position(exchanges.SANDBOX, 'BTCUSD')
    assert p.is_close
