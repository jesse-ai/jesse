...
def get_btc_and_eth_candles():
    ...
    candles[jh.key(Exchanges.SANDBOX.value, 'ETH-USDT')] = {
        'exchange': Exchanges.SANDBOX.value,
        'symbol': 'ETH-USDT',
...
def set_up(is_futures_trading=True, leverage=1, leverage_mode='cross', fee=0):
    reset_config()
    config['env']['exchanges'][Exchanges.SANDBOX.value]['balance'] = 10_000
    ...
    if is_futures_trading:
        ...
        config['env']['exchanges'][Exchanges.SANDBOX.value]['futures_leverage_mode'] = leverage_mode
        config['env']['exchanges'][Exchanges.SANDBOX.value]['futures_leverage'] = leverage
    else:
...
def two_routes_backtest(
    ...
    # dates are fake. just to pass required parameters
    backtest_mode.run('000', False, {}, Exchanges.SANDBOX.value, routes, [], '2019-04-01', '2019-04-02', get_btc_and_eth_candles())
