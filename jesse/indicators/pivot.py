...
# Demark Pivot Points
        ...
        p = np.where(close<open,(high+2*low+close)/4,np.where(close>open,(2*high+low+close)/4,np.where(close==open,(high+low+2*close)/4,np.nan)))
        s1 = np.where(close<open,(high+2*low+close)/2 - high,np.where(close>open,(2*high+low+close)/2 - high,np.where(close==open,(high+low+2*close)/2 - high,np.nan)))
        r1 = np.where(close<open,(high+2*low+close)/2 - low,np.where(close>open,(2*high+low+close)/2 - low,np.where(close==open,(high+low+2*close)/2 - low,np.nan)))
    elif (mode ==3):
