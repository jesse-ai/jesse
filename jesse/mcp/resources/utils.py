"""
Jesse MCP Utilities Resources

This module provides comprehensive documentation for Jesse's utility functions
through the MCP (Model Context Protocol). It serves as the complete reference
for all helper functions available in jesse.utils that are essential for strategy
development.

The registered resource includes:
- Position sizing utilities (size_to_qty, risk_to_qty, qty_to_size)
- Risk calculation helpers (estimate_risk, limit_stop_loss)
- Series analysis functions (crossed, strictly_increasing, streaks)
- Statistical tools (kelly_criterion, z_score, prices_to_returns)
- Data conversion utilities (numpy_candles_to_dataframe)
- Mathematical precision helpers (subtract_floats, sum_floats)
- Timeframe utilities (anchor_timeframe)

These utilities are fundamental building blocks for implementing robust trading strategies.
"""

def register_utils_resources(mcp):

    @mcp.resource("jesse://utilities")
    def utilities():
        """
        Get comprehensive reference for Jesse utility functions.

        This reference covers all helper functions available in jesse.utils
