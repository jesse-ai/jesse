...
def register_backtest_tools(mcp):
    ...
    def create_backtest_draft(
        ...
        try:
            ...
            # Update the session state with the draft config
    ...
    def update_backtest_draft(backtest_id: str, state: str) -> dict:
        ...
        try:
            ...
            # Update the session state with the modified state
            from hashlib import sha256
            auth_token_hashed = sha256(auth_token.encode('utf-8')).hexdigest()
    ...
    @mcp.tool()
    def run_backtest(session_id: str, config: str, timeout_seconds: int = 24 * 60 * 60) -> dict:
        """
        ...
        3. use get_backtest_config() → get backtest config response
        4. run_backtest(session_id, response['config']) → pass config JSON to run_backtest
        5. Monitor progress via WebSocket events until completion

        Args:
            session_id: ID of the backtest session to run
            config: Backtest configuration as JSON string from
        ...
        try:
            ...
            # Parse the provided backtest configuration
            import json
            try:
                backtest_config = json.loads(config)
            except json.JSONDecodeError:
                return {
                    'status': 'error',
                    'message': 'Invalid JSON format for config parameter'
                }
            ...
            try:
                ...
                while elapsed_time < max_wait_time:
                    ...
                    if local_state['completed']:
                        ...
                        final_session = get_backtest_session(session_id)
